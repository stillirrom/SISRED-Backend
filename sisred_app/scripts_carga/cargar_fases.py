...
django.setup()
from sisred_app.models import Fase

#Metodo para cargar los roles desde el archivo plano
with open('./archivos/fases.csv', encoding='utf-8') as csv_file:
    ...
    for row in csv_reader:
        if line_count == 0:
            ...
            id_conectate = row[0]
            nombre = row[1]
            obj, created = Fase.objects.update_or_create(
                id_conectate=id_conectate, defaults={'nombre_fase': nombre}
            )
            line_count += 1
